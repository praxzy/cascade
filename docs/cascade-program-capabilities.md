...
| `close_stream`                | Closes both the stream PDA and the vault account, refunding rent to the employer.                                                                                                                                  | Stream must already be inactive and the vault balance must be zero.                                             |
...
- All value transfers go through CPI calls against `anchor_spl::token::transfer`.
